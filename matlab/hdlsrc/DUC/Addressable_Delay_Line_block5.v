// -------------------------------------------------------------
// 
// File Name: hdlsrc\DUC\Addressable_Delay_Line_block5.v
// Created: 2025-01-02 14:32:56
// 
// Generated by MATLAB 24.2, HDL Coder 24.2, and Simulink 24.2
// 
// -------------------------------------------------------------


// -------------------------------------------------------------
// 
// Module: Addressable_Delay_Line_block5
// Source Path: DUC/DUC_module/Halfband Filter 1/Addressable_Delay_Line
// Hierarchy Level: 2
// Model version: 3.26
// 
// Addressable Delay Line
// 
// -------------------------------------------------------------

`timescale 1 ns / 1 ns

module Addressable_Delay_Line_block5
          (clk,
           rst_n,
           enb,
           dataIn,
           validIn,
           wrAddr,
           rdAddr,
           delayLineEnd,
           dataOut);


  input   clk;
  input   rst_n;
  input   enb;
  input   signed [31:0] dataIn;  // sfix32_En30
  input   validIn;
  input   [1:0] wrAddr;  // ufix2
  input   [1:0] rdAddr;  // ufix2
  output  signed [31:0] delayLineEnd;  // sfix32_En30
  output  signed [31:0] dataOut;  // sfix32_En30


  reg [1:0] wrAddrREG;  // ufix2
  wire saveLast;
  reg  dataEndEn;
  wire wrEnN;
  wire dataEndEnS;
  wire signed [31:0] delayedSignals;  // sfix32_En30
  reg signed [31:0] dataOutReg_reg [0:1];  // sfix32 [2]
  wire signed [31:0] dataOutReg_reg_next [0:1];  // sfix32_En30 [2]
  reg signed [31:0] dataOut_1;  // sfix32_En30
  reg signed [31:0] dataOutReg_t_0_0;  // int32
  reg signed [31:0] dataOutReg_t_1;  // int32


  always @(posedge clk)
    begin : dataOutReg_process
      if (rst_n == 1'b0) begin
        wrAddrREG <= 2'b00;
      end
      else begin
        if (enb) begin
          wrAddrREG <= wrAddr;
        end
      end
    end

  assign saveLast = wrAddr != wrAddrREG;

  always @(posedge clk)
    begin : dataOutReg_1_process
      if (rst_n == 1'b0) begin
        dataEndEn <= 1'b0;
      end
      else begin
        if (enb) begin
          dataEndEn <= saveLast;
        end
      end
    end

  assign wrEnN =  ~ dataEndEn;

  assign dataEndEnS = saveLast & wrEnN;

  SimpleDualPortRAM_generic #(.AddrWidth(2),
                              .DataWidth(32)
                              )
                            u_simpleDualPortRam (.clk(clk),
                                                 .enb(enb),
                                                 .wr_din(dataIn),
                                                 .wr_addr(wrAddr),
                                                 .wr_en(validIn),
                                                 .rd_addr(rdAddr),
                                                 .dout(delayedSignals)
                                                 );

  always @(posedge clk)
    begin : dataOutReg_2_process
      if (rst_n == 1'b0) begin
        for(dataOutReg_t_1 = 32'sd0; dataOutReg_t_1 <= 32'sd1; dataOutReg_t_1 = dataOutReg_t_1 + 32'sd1) begin
          dataOutReg_reg[dataOutReg_t_1] <= 32'sb00000000000000000000000000000000;
        end
      end
      else begin
        if (enb && dataEndEnS) begin
          for(dataOutReg_t_0_0 = 32'sd0; dataOutReg_t_0_0 <= 32'sd1; dataOutReg_t_0_0 = dataOutReg_t_0_0 + 32'sd1) begin
            dataOutReg_reg[dataOutReg_t_0_0] <= dataOutReg_reg_next[dataOutReg_t_0_0];
          end
        end
      end
    end

  assign delayLineEnd = dataOutReg_reg[1];
  assign dataOutReg_reg_next[0] = delayedSignals;
  assign dataOutReg_reg_next[1] = dataOutReg_reg[0];

  always @(posedge clk)
    begin : reg_rsvd_process
      if (rst_n == 1'b0) begin
        dataOut_1 <= 32'sb00000000000000000000000000000000;
      end
      else begin
        if (enb) begin
          dataOut_1 <= delayedSignals;
        end
      end
    end

  assign dataOut = dataOut_1;

endmodule  // Addressable_Delay_Line_block5

